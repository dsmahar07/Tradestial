              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-[#2a2a2a]">
                <thead className="bg-gray-50 dark:bg-[#2a2a2a] sticky top-0">
                  <tr>
                    ...
                    <th onClick={() => requestSort('symbol')} className="px-4 p-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-[#333]">
...
export function AccountManagementClient() {
  ...
  const totalStats = useMemo(() => {
    if (!isHydrated) {
      return { totalAccounts: 0, totalTrades: 0, totalPnl: 0, avgWinRate: 0 }
    }
    return accountService.getCombinedStats()
  }, [accounts, isHydrated]);
